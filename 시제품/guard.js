...
// 탭 셋 — 거리순(할 일 전체) · 일정순(방문 날짜가 가까운 순) · 재점검(공장주가 고쳤다고 한 곳만) (2026-09-22 사용자 결정)
const SORTS = [['near', '거리순'], ['date', '일정순'], ['re', '재점검']];
...
function facRow(s, fid, tk) {
  ...
  const now = tk.map((x) => (x.kind === 'back' ? '반려' : x.kind === 're' ? `재점검 ${x.n}개` : '첫 점검')).join(', ');
